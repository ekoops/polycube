...
static inline int send_to_frontend(struct CTXTYPE *ctx, struct pkt_metadata *md, __be32 ip_daddr) {
    if (SINGLE_PORT_MODE) {
        pcn_log(ctx, LOG_TRACE, "sent pkt to the FRONTEND port - (out_port: %d)", FRONTEND_PORT);
        return pcn_pkt_redirect(ctx, md, FRONTEND_PORT);
    }
    u16 *fe_port = ip_to_frontend_port.lookup(&ip_daddr);
    if (!fe_port) {
        pcn_log(ctx, LOG_ERR, "failed FRONTEND port lookup for %I", ip_daddr);
...
static __always_inline int handle_rx(struct CTXTYPE *ctx,
    ...
    if (md->in_port != BACKEND_PORT) {
        pcn_log(ctx, LOG_TRACE, "received pkt is from a FRONTEND port - (in_port %d)", md->in_port);
        ...
        *     SESSION TABLE
        */
        pcn_log(ctx, LOG_TRACE, "received pkt is from the BACKEND port - (in_port %d)", md->in_port);
        ...
        if (v && v->sense == FROM_BACKEND) {
            ...
            if (!vip_v) {
                pcn_log(ctx, LOG_ERR, "failed reverse backend lookup");
                goto DROP;
            }
            ...
            __u32 check = jhash((const void *) &sessions_key, sizeof(struct sessions), JHASH_INITVAL);

            pcn_log(ctx, LOG_TRACE, "check in session table %lu", check);

            // Let's check if this session is present in the session table
            struct vip *rev_proxy = hash_session.lookup(&check);

            // If this (return) packet belongs to a session that was handled by the
            // LB, we have to  translate IP addresses back to their original value
            if (!rev_proxy) {
                pcn_log(ctx, LOG_TRACE, "failed session lookup - trying to redirect to a proper FRONTEND port");
        ...
        u16 fe_port;
        if (SINGLE_PORT_MODE) fe_port = FRONTEND_PORT;
        else {
            __be32 fe_ip = ip->daddr;
            u16 *fe_port_ptr = ip_to_frontend_port.lookup(&fe_ip);
            if (!fe_port_ptr) {
                pcn_log(ctx, LOG_ERR, "failed FRONTEND port lookup");
                goto DROP;
            }
    ...
    if (md->in_port == BACKEND_PORT) {
        pcn_log(ctx, LOG_TRACE, "received ARP pkt from BACKEND port - (in_port: %d)", md->in_port);
        ...
        pcn_log(ctx, LOG_TRACE, "received ARP pkt from FRONTEND port - (in_port: %d)", md->in_port);
        if (arp->ar_op == bpf_htons(ARPOP_REPLY)) {
            pcn_log(
                    ctx, LOG_TRACE, "received ARP pkt is an ARP reply - (in_port: %d) (arp_opcode: %d)",
                    md->in_port, arp->ar_op
            );
            ...
            if (v && v->sense == FROM_FRONTEND) {
                ...
                pcn_log(ctx, LOG_TRACE, "sent received ARP reply to slowpath - (in_port: %d)", md->in_port);
                pcn_pkt_controller(ctx, md, REASON_ARP_REPLY);
                return RX_DROP;
            }
        }
        pcn_log(
                ctx, LOG_TRACE, "redirected ARP pkt to BACKEND port - (in_port: %d) (out_port: %d)",
    ...
    if (md->in_port == BACKEND_PORT) {
        pcn_log(
                ctx, LOG_TRACE,
                "received non-ip packet from BACKEND port: sent to slowpath - (in_port: %d) (proto: 0x%x)",
        ...
        pcn_log(
                ctx, LOG_TRACE,
                "received non-ip packet from FRONTEND port: redirected to BACKEND_PORT - (in_port: %d) (proto: 0x%x) (out_port: %d)",
    ...
    DROP:
    if (md->in_port == BACKEND_PORT) {
        pcn_log(ctx, LOG_TRACE, "dropped packet from BACKEND port - (in_port: %d)", md->in_port);
    } else {
        pcn_log(ctx, LOG_TRACE, "dropped packet from FRONTEND port - (in_port: %d)", md->in_port);
    }
    return RX_DROP;
}
